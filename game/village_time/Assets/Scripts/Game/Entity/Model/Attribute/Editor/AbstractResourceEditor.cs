...
[CustomEditor(typeof(AbstractResource), true)]
[CanEditMultipleObjects]
public class AbstractResourceEditor : Editor {
    private float inputValue = 0f;

    public override void OnInspectorGUI() {
        var resource = (AbstractResource)this.target;
        ...
        if (GUILayout.Button("-", GUILayout.Width(25))) {
            ...
            ((AbstractResource)this.target).Reduce(this.inputValue);
            EditorUtility.SetDirty(this.target);
        }

        if (GUILayout.Button("+", GUILayout.Width(25))) {
            Undo.RecordObject(this.target, "增加资源值");
            ((AbstractResource)this.target).Increase(this.inputValue);
            EditorUtility.SetDirty(this.target);
        }

        if (GUILayout.Button("=", GUILayout.Width(25))) {
            Undo.RecordObject(this.target, "设置资源值");
            ((AbstractResource)this.target).Change(this.inputValue);
            EditorUtility.SetDirty(this.target);
        }
